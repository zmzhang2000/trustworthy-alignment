...
def create_hf_model(model_class,
    ...
    if rlhf_training:
        ...
        flash_attn_kargs = {"use_flash_attention_2": True} if "llama" in model_name_or_path.lower() else {}
        model = model_class.from_pretrained(
            model_name_or_path,
            torch_dtype="auto", **flash_attn_kargs,
            from_tf=bool(".ckpt" in model_name_or_path),
            config=model_config)
        if "fp16" in ds_config:
            model = model.half()
        elif "bfloat16" in ds_config:
...
def create_critic_model(model_name_or_path,
    ...
    if "fp16" in ds_config:
        critic_model = critic_model.half()
    elif "bfloat16" in ds_config:
        critic_model = critic_model.bfloat16()
    else:
        critic_model = critic_model.float() 
    return critic_model
