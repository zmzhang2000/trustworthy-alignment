...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    elif "MRQANaturalQuestions" in dataset_name:
        return raw_datasets.MRQANaturalQuestionsDataset(output_path, seed, local_rank, dataset_name)
    else:
        raise RuntimeError(
    ...
    def __getitem__(self, idx):
        if self.train_phase == 1:
            ...
            return self.prompt_dataset[idx]["input_ids"],self.prompt_dataset[idx]["attention_mask"], \
                self.pad_token_id, self.prompt_dataset[idx]["right_answer"], self.prompt_dataset[idx]["wrong_answer"]
    ...
    if train_phase == 1:
        ...
        for i, tmp_data in enumerate(current_dataset):
            ...
            if prompt is not None:
                ...
                prompt_token["right_answer"] = raw_dataset.get_right_answer(tmp_data)
                prompt_token["wrong_answer"] = raw_dataset.get_wrong_answer(tmp_data)
                if prompt_token["input_ids"].size()[-1] <= max_seq_len:
                    ...
                    # filtered += 1
                    # choose to truncate the context rather than filtering out the data
                    ...
                    ids_num_to_delete = prompt_token["input_ids"].size()[-1] - max_seq_len
                    for key_word in ["input_ids", "attention_mask"]:
                        y = prompt_token[key_word].squeeze(0)
                        y = torch.cat((y[:-keep_end_ids_num][:-ids_num_to_delete], y[-keep_end_ids_num:]))
                        assert len(y) == max_seq_len
                        prompt_token[key_word] = y.flip(0)
                    prompt_dataset.append(prompt_token)
...
class DataCollatorRLHF:
    ...
    def __call__(self, data):
        ...
        batch["right_answer"] = [f[3] for f in data]
        batch["wrong_answer"] = [f[4] for f in data]
        return batch
