...
def detect_periodic_substring(s: str):
    """
    Detects the longest periodic substring of a given string.
    This is a brute force algorithm with time complexity O(n^3).
    It should be optimized if `s` is longer than 100.
    """
    pattern2count = {}
    # For each substring
    n = len(s)
    for i in range(0, n):
        for j in range(i, n):
            ...
            m = len(substring)
            for k in range(1, m // 2 + 1): # The longest periodic pattern length is m // 2
                pattern = substring[:k]
                if m % k == 0 and pattern * (m // k) == substring:
                    pattern2count[pattern] = max(m // k, pattern2count.get(pattern, 0))
                    break # No need to check other patterns including shorter one
    return pattern2count


def max_T_periodic_substring(s: str) -> int:
    if len(s) == "":
        return 0
    pattern2count = detect_periodic_substring(s)
    if len(pattern2count) == 0:
        return 1
    else:
        return max(pattern2count.values())
