...
from dschat.utils.data.data_utils import create_prompt_dataset, MiniDataset, DataCollatorRLHF, get_unsupervised_data, get_raw_dataset
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="How many steps to run evaluation on the eval dataset.")
...
def create_eval_dataset(args, tokenizer):
    eval_raw_dataset = get_raw_dataset(args.data_path[0], args.data_output_path,
                                       args.seed, args.local_rank).get_eval_data()
    
    def fn(sample):
        # sample.update(self.tokenizer(sample["text"], add_special_tokens=False, return_tensors="pt", max_length=10, truncation=True))
        sample.update(tokenizer(sample["text"], add_special_tokens=False, return_tensors="pt"))
        
        max_seq_len = args.max_prompt_seq_len
        if sample["input_ids"].size()[-1] > max_seq_len:
            # truncate the context rather than filtering out the data
            keep_end_ids = tokenizer.encode("\nA:", add_special_tokens=False)[1:] # llama tokenizer adds a special token 29871 to the start of the string
            keep_end_ids_num = len(keep_end_ids)
            ids_num_to_delete = sample["input_ids"].size()[-1] - max_seq_len
            for key_word in ["input_ids", "attention_mask"]:
                y = sample[key_word].squeeze(0)
                y = torch.cat((y[:-keep_end_ids_num][:-ids_num_to_delete], y[-keep_end_ids_num:]))
                assert len(y) == max_seq_len, f"len(y)={len(y)}, max_seq_len={max_seq_len})"
                sample[key_word] = y.unsqueeze(0)
        return sample

    eval_dataset = eval_raw_dataset.map(fn, load_from_cache_file=False)
    
    def collate_fn(samples):
        batch = {}
        batch["text"] = [x["text"] for x in samples]
        batch["input_ids"] = torch.cat([torch.tensor(x["input_ids"]) for x in samples], dim=0)
        batch["attention_mask"] = torch.cat([torch.tensor(x["attention_mask"]) for x in samples], dim=0)
        batch["right_answer"] = [x["right_answer"] for x in samples]
        batch["wrong_answer"] = [x["wrong_answer"] for x in samples]
        return batch
    ...
    dataloader = DataLoader(eval_dataset, collate_fn=collate_fn, sampler=sampler, batch_size=1)
    return dataloader
...
def main():
    ...
    eval_dataloader = create_eval_dataset(args=args, tokenizer=tokenizer)
    ...
    step_average_reward, ema_reward_score = 0., ExponentialMovingAverage()
    step_rl_info, ema_rl_info = defaultdict(float), defaultdict(ExponentialMovingAverage)
    step_metrics, ema_metrics = defaultdict(float), defaultdict(ExponentialMovingAverage)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
                zip(prompt_train_dataloader, unsupervised_train_dataloader, strict=True)):
            ...
            out = trainer.generate_experience(batch_prompt['prompt'],
                                            ...
                                            step,
                                            batch_prompt['right_answer'],
                                            batch_prompt['wrong_answer'])
            ...
            if exp_dataset is not None:
                ...
                average_reward = 0
                average_rl_info = defaultdict(float)
                ...
                for ppo_ep in range(args.ppo_epochs):
                    for i, (exp_data, unsup_data) in enumerate(
                            zip(exp_dataset, unsup_dataset, strict=True)):
                        actor_loss, critic_loss = trainer.train_rlhf(exp_data)
                        actor_loss_sum += actor_loss.item()
                        critic_loss_sum += critic_loss.item()
                        average_reward += exp_data["rewards"].mean()
                        for k, v in exp_data.items():
                            if "response_" in k or "temperature" == k:
                                average_rl_info[k] += v.float().mean()
                            elif "_ratio" in k:
                                average_metrics[k] += v.float().mean()
                ...
                for k, v in average_rl_info.items():
                    average_rl_info[k] = get_all_reduce_mean(v).item()
                for k, v in average_metrics.items():
                    average_metrics[k] = get_all_reduce_mean(v).item()

                step_average_reward += average_reward / args.gradient_accumulation_steps_actor
                for k, v in average_rl_info.items():
                    step_rl_info[k] += v / args.gradient_accumulation_steps_actor
                for k, v in average_metrics.items():
                    step_metrics[k] += v / args.gradient_accumulation_steps_actor
            
                if (step + 1) % args.gradient_accumulation_steps_actor == 0:
                    ema_reward_score.update(step_average_reward)
                    step_average_reward = 0.
                    for k, v in step_rl_info.items():
                        ema_rl_info[k].update(v)
                        step_rl_info[k] = 0.
                    for k, v in step_metrics.items():
                        ema_metrics[k].update(v)
                        step_metrics[k] = 0.
                ...
                if args.enable_tensorboard and torch.distributed.get_rank(
                    ...
                    writer.add_scalar('rl_info/reward',
                                      average_reward / inner_iter,
                                      global_step=step + epoch * len(prompt_train_dataloader))
                    for k, v in average_rl_info.items():
                        writer.add_scalar(f'rl_info/{k}',
                                          v / inner_iter,
                                          global_step=step + epoch * len(prompt_train_dataloader))
                    for k, v in average_metrics.items():
                        writer.add_scalar(f'experiences_metric/{k}',
                                          v / inner_iter,
                                          global_step=step + epoch * len(prompt_train_dataloader))
                    writer.add_scalar('loss/actor_lr',
                                      rlhf_engine.actor.get_lr()[0],
                                      global_step=step + epoch * len(prompt_train_dataloader))
                    writer.add_scalar('loss/actor_lora_lr',
                                      rlhf_engine.actor.get_lr()[1],
                                      global_step=step + epoch * len(prompt_train_dataloader))
                    writer.add_scalar('loss/actor_loss',
                                      actor_loss.item(),
                                      global_step=step + epoch * len(prompt_train_dataloader))
                    writer.add_scalar('loss/actor_loss_sum',
                                      actor_loss_sum,
                                      global_step=step + epoch * len(prompt_train_dataloader))
                    writer.add_scalar('loss/critic_lr',
                                      rlhf_engine.critic.get_lr()[0],
                                      global_step=step + epoch * len(prompt_train_dataloader))
                    writer.add_scalar('loss/critic_lora_lr',
                                      rlhf_engine.critic.get_lr()[1],
                                      global_step=step + epoch * len(prompt_train_dataloader))
                    writer.add_scalar('loss/critic_loss',
                                      critic_loss.item(),
                                      global_step=step + epoch * len(prompt_train_dataloader))
                    writer.add_scalar('loss/critic_loss_sum',
                                      critic_loss_sum,
                                      global_step=step + epoch * len(prompt_train_dataloader))
                    writer.flush()
            ...
            if step % args.eval_steps == 0 and step != 0:
                trainer.evaluate(eval_dataloader, step + epoch * len(prompt_train_dataloader), device, writer)
